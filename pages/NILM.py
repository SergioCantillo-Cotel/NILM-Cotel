...
try:
    fecha_ini, fecha_fin = st.sidebar.date_input("Periodo", (pd.Timestamp.now() - pd.Timedelta(weeks=1), pd.Timestamp.now()), min_value='2025-05-15', max_value=pd.Timestamp.now(), key='rango_fecha_NILM')
    ...
    fecha_ini, fecha_fin = pd.Timestamp.now() - pd.Timedelta(weeks=1), pd.Timestamp.now()
